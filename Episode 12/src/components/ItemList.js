...
import { addItem, removeItem, updateCount } from "../util/cartSlice";
...
const ItemList = ({ itemData }) => {
    ...
    const handleAddClick = (count, itemData) => {
        setCount(count);
        const itemIndex = cartItems.findIndex(item => item?.name == itemData?.name);
        if (itemIndex >= 0){
            if(count == 0)
        ...
        <div className="flex justify-between my-4 px-2 border-b border-solid py-1 border-gray-300">
            ...
            <div className="w-3/12 relative">
                ...
                {count == 0 && <button onClick={() => {
                    handleAddClick(count + 1, itemData);
